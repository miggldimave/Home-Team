...
import type { Category, ScoringMode } from '@/lib/types'

export function AddTaskForm({ categories: initialCategories, scoringMode }: { categories: Category[]; scoringMode: ScoringMode }) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()

  const [categories, setCategories] = useState<Category[]>(initialCategories)
  const [name, setName] = useState('')
  const [selectedCat, setSelectedCat] = useState(initialCategories[0]?.name ?? '')
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, padding: '0 0 60px', position: 'relative' }}>
      {/* Header */}
      <div style={{ padding: '60px 24px 0', display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={() => router.back()} style={{ background: 'rgba(255,255,255,0.75)', border: 'none', cursor: 'pointer', width: 36, height: 36, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          {Icons.back(18, txt)}
        </button>
        <div style={{ fontFamily: '"Instrument Serif", Georgia, serif', fontSize: 34, color: txt, letterSpacing: -0.4, lineHeight: 1.05 }}>Aufgabe hinzufügen</div>
      </div>
      ...
      <div style={{ margin: '10px 16px 0', padding: '20px', borderRadius: 24, background: cardBg, border: cardBorder }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
          {categories.map((c) => (
            <button
              ...
              style={{
                ...
                background: selectedCat === c.name ? c.hue : 'rgba(0,0,0,0.04)',
                color: selectedCat === c.name ? '#fff' : muted,
          ...
          <button
            ...
            style={{ padding: '8px 14px', borderRadius: 999, border: '1px dashed rgba(0,0,0,0.15)', cursor: 'pointer', fontSize: 13, fontWeight: 500, background: 'transparent', color: muted, display: 'flex', alignItems: 'center', gap: 6 }}
          >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          {AVAILABLE_ICONS.map((ic) => (
            <button
              ...
              onClick={() => setIcon(ic)}
              style={{
                width: 42, height: 42, borderRadius: 13, border: 'none', cursor: 'pointer', fontSize: 20,
                background: icon === ic ? (selectedCatObj?.soft ?? 'rgba(0,0,0,0.06)') : 'rgba(0,0,0,0.04)',
                color: icon === ic ? (selectedCatObj?.deep ?? txt) : muted,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: icon === ic ? `0 0 0 2px ${selectedCatObj?.hue ?? txt}` : 'none',
                transition: 'all 0.15s',
              }}
            >
              {ic}
      ...
      <div style={{ margin: '10px 16px 0', padding: '20px', borderRadius: 24, background: cardBg, border: cardBorder }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <label>
            ...
            <input
              type="number" min="1" value={cycleDays} onChange={(e) => setCycleDays(e.target.value)}
              style={{ width: '100%', padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(0,0,0,0.08)', background: 'rgba(0,0,0,0.02)', fontSize: 15, color: txt, outline: 'none', boxSizing: 'border-box' }}
            />
          </label>
          {scoringMode === 'punkte' && (
      ...
      <div style={{ margin: '16px 16px 0' }}>
        <button
          ...
          disabled={isPending}
          style={{ width: '100%', padding: '16px', borderRadius: 18, border: 'none', cursor: 'pointer', background: txt, color: '#FDF8F1', fontSize: 15, fontWeight: 600 }}
        >
          {isPending ? 'Speichern…' : 'Aufgabe erstellen'}
        </button>
      </div>
      ...
      {showCatModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 200, display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ width: '100%', background: bg, borderRadius: '24px 24px 0 0', padding: '28px 20px 40px' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                {CATEGORY_COLOR_OPTIONS.map((col, i) => (
                  <button
                    ...
                    style={{ width: 34, height: 34, borderRadius: '50%', border: 'none', cursor: 'pointer', background: col.hue, boxShadow: newCatColorIdx === i ? `0 0 0 3px ${bg}, 0 0 0 5px ${col.hue}` : 'none', transition: 'box-shadow 0.15s' }}
                  />
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setShowCatModal(false); setCatError('') }} style={{ flex: 1, padding: '13px', borderRadius: 14, border: '1px solid rgba(0,0,0,0.08)', cursor: 'pointer', background: 'transparent', color: muted, fontSize: 14, fontWeight: 500 }}>
                Abbrechen
              </button>
              <button onClick={handleCreateCategory} disabled={isPending} style={{ flex: 2, padding: '13px', borderRadius: 14, border: 'none', cursor: 'pointer', background: txt, color: '#FDF8F1', fontSize: 14, fontWeight: 600 }}>
